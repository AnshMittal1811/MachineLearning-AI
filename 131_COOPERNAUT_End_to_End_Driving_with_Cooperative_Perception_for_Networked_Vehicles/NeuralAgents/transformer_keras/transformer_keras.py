...
# from AutoCastSim.srunner.challenge.autoagents.transformer_keras.dataloader import TokenList, pad_to_longest
from NeuralAgents.transformer_keras.attention_keras import *
...
class MultiHeadAttention():
    ...
    def __init__(self, n_head, d_model, dropout, len=30, mode=0):
        self.mode = mode
        self.n_head = n_head
        self.d_k = self.d_v = d_k = d_v = d_model // n_head
        self.dropout = dropout
        # self.len_q = self.len_v = self.len_k = len
        self.len_qkv = len
        if mode == 0:
    ...
    def __call__(self, q, k, v, mask=None):
        ...
        if self.mode == 0:
            ...
            def reshape1(x):
                ...
                x = tf.reshape(x, [-1, len_qkv, n_head, d_k])
                x = tf.transpose(x, [2, 0, 1, 3])
                # x = tf.reshape(x, [-1, s[1], s[2] // n_head])  # [n_head * batch_size, len_q, d_k]
                x = tf.reshape(x, [-1, len_qkv, d_k])  # [n_head * batch_size, len_q, d_k]
                return x

            def reshape1_layer(x, len_qkv):

                # x = Reshape([-1, len_qkv, n_head, d_k])(x)
                # x = Permute([2, 0, 1, 3])(x)
                # x = Reshape([-1, len_qkv, d_k])(x)
                x = K.reshape(x, [-1, len_qkv, n_head, d_k])
                x = K.permute_dimensions(x, [2, 0, 1, 3])
                x = K.reshape(x, [-1, len_qkv, d_k])
                return x

            # qs = Lambda(reshape1)(qs)
            # ks = Lambda(reshape1)(ks)
            # vs = Lambda(reshape1)(vs)
            qs = reshape1_layer(qs, len_qkv)
            ks = reshape1_layer(ks, len_qkv)
            vs = reshape1_layer(vs, len_qkv)
            ...
            def reshape2(x):
                ...
                x = tf.reshape(x, [n_head, -1, len_qkv, d_v])
                x = tf.transpose(x, [1, 2, 0, 3])
                # x = tf.reshape(x, [-1, s[1], n_head * d_v])  # [batch_size, len_v, n_head * d_v]
                x = tf.reshape(x, [-1, len_qkv, n_head * d_v])  # [batch_size, len_v, n_head * d_v]
                return x

            # head = Lambda(reshape2)(head)

            # head = Reshape([n_head, -1, len_qkv, d_v])(head)
            # head = Permute([1, 2, 0, 3])(head)
            # head = Reshape([-1, len_qkv, n_head * d_v])(head)
            head = K.reshape(head, [n_head, -1, len_qkv, d_v])
            head = K.permute_dimensions(head, [1, 2, 0, 3])
            head = K.reshape(head, [-1, len_qkv, n_head * d_v])
...
class EncoderLayer():
    def __init__(self, d_model, d_inner_hid, n_head, len_qkv, dropout=0.1):
        # self.self_att_layer = MultiHeadAttention(n_head, d_model, dropout=dropout,  len=len_qkv)
        self.self_att_layer = MultiHeadSelfAttention(num_heads=n_head, use_masking=True, dropout=dropout)
        self.pos_ffn_layer = PositionwiseFeedForward(d_model, d_inner_hid, dropout=dropout)
        self.norm_layer = LayerNormalization()

    def __call__(self, enc_input, mask=None):
        # output, slf_attn = self.self_att_layer(enc_input, enc_input, enc_input, mask=mask)
        output = self.self_att_layer(enc_input)
...
class SelfAttention():
    def __init__(self, d_model, d_inner_hid, n_head, len_qkv, layers=6, dropout=0.1):
        self.layers = [EncoderLayer(d_model, d_inner_hid, n_head, len_qkv=len_qkv, dropout=dropout) for _ in range(layers)]

    # def __call__(self, src_emb, src_seq, return_att=False, active_layers=999, maskval=0):
    #     if return_att: atts = []
    #     mask = Lambda(lambda x: K.cast(K.greater(x, maskval), 'float32'))(src_seq)
    #     x = src_emb
    ...
    def __call__(self, src_emb, mask, return_att=False, active_layers=999, maskval=0):
        if return_att: atts = []
        # mask = Lambda(lambda x: K.cast(K.greater(x, maskval), 'float32'))(src_seq)
        x = src_emb
        for enc_layer in self.layers[:active_layers]:
