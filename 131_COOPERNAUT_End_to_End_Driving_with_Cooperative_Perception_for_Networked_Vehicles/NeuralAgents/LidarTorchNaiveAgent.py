...
class LidarTorchNaiveAgent(AutonomousAgent):
    
    def __init__(self, path_to_conf_file):
        super().__init__(path_to_conf_file)
        self.model_path = 'training/lidar_only_fixed/model-256.th'
        self.num_output = 3
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu') 
        net = NaiveModel(self.num_output).to(self.device)
        net.eval()
    ...
    def run_step(self, input_data, timestamp, JSONState=None):
        ...
        control = super(LidarTorchNaiveAgent, self).run_step(input_data, timestamp)
        ...
        if JSONState is not None:
            ...
            if self._final_goal:
                ...
                pred_location = self.net(BEV)
                pred_location = pred_location.cpu().detach().numpy()
                pred_location[2] = 0 #setting the z to be 0 to avoid possible bug
                if fused_points.id==-1:
