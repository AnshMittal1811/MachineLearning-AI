...
class LiDARDataset(Dataset):
    def __init__(self, config):
        ...
        self.z_min = config.z_min if hasattr(config, "z_min") else -2.5
        self.z_max = config.z_max if hasattr(config, "z_max") else -0.3
    ...
    def _pad_or_truncate(self, points, npoints=65536):
        if points is None: 
            print(points) 
            return None
    ...
    def __getitem__(self, idx):
        while True:
            ...
            if parsed_measurements:
                ...
                if not self.earlyfusion:
                    #lidar = LidarPreprocessor.Sparse_Quantize(lidar, z_min=self.z_min, z_max=self.z_max)
                    lidar = LidarPreprocessor.Sparse_Quantize(lidar)
                    ego_lidar = np.unique(lidar, axis=0)
                    ego_lidar = self._pad_or_truncate(ego_lidar, self.npoints)
                else:
                ...
                ego_transform = np.array(ego_transform.inversematrix()) # ego transform should be inverse transform
                while num_valid_neighbors < self.max_num_neighbors: #filling up neighbors' observations according to distance
                    try:
                        ...
                        other_z_compensation = 2*abs(other_agents[other_agent_id]['bounding_box']['extent_z'])+Utils.LidarRoofTopDistance
                        ...
                        if num_neighbors>=len(other_agents_keys_dist):
                            other_agent_lidar.append(np.zeros_like(ego_lidar))
                            other_agent_speed.append(0.0)
                            other_agent_transform.append(np.eye(4)) 
                    num_neighbors += 1
        ...
        if self.earlyfusion:
            ...
            ego_lidar = LidarPreprocessor.Sparse_Quantize(ego_lidar)
            ego_lidar = np.unique(ego_lidar, axis=0)
            ego_lidar = self._pad_or_truncate(ego_lidar, 2*self.npoints) # used to be 2* npoints
...
if __name__== '__main__':
    ...
    parser.add_argument('--npoints', type=int, default=1024, help='how many points to downsample to')
    ...
    for batch_data in tqdm.tqdm(dataset):
        ...
        for x in other_transform[0]:
            if abs(x[0][0]-1) >0.1 or abs(x[1][1]-1) >0.1 or abs(x[2][2]-1)>0.1:
                ...
                #from matplotlib import pyplot as plt; from AutoCastSim.AVR.PCProcess import LidarPreprocessor; import numpy as np; bev = LidarPreprocessor.Lidar2BEV_v2(ego_lidar[0].detach().cpu().numpy( )); bev_show = np.mean(bev, axis=2); plt.imshow(bev_show); plt.show()
        #if i>=3: break
        #if i>=30: break 
