...
"""Torch implementation of SMPL model inherited from https://github.com/CalciferZh/SMPL/blob/master/smpl_torch.py"""


class SMPLModel(Module):
    def __init__(self, device=None, model_path='models/basicmodel_m_lbs_10_207_0_v1.0.0.pkl'):
        ...
        self.shapedirs = torch.from_numpy(np.asarray(params['shapedirs'])).type(torch.float64)
...
# Global variables
id2names = {0: 'male', 1: 'female'}
id2ckpt = {0: 'basicmodel_m_lbs_10_207_0_v1.0.0.pkl',
           1: 'basicModel_f_lbs_10_207_0_v1.0.0.pkl'}
...
# TODO(mattiasegu): (0.2 and 0.03) OR (0.4 and 0.06)?
POSE_STDDEV = 0.4
BETAS_STDDEV = 0.06


def generate_smpl(opt, class_id=0, gpu_id=[0]):
    ...
    # TODO(mattiasegu): add option to avoid creating this dataset if it already exists and has exactly TRAINING_SET_SIZE
    ...
    # Load SMPL model
    model_path = os.path.join(opt.models_dir, id2ckpt[class_id])
    if os.path.exists(model_path):
        print('Loading SMAL model from {}'.format(model_path))
        model = SMPLModel(device=device, model_path=model_path)
    else:
        raise ValueError('Path not found: {}'.format(model_path))
    ...
    for i in range(TRAINING_SET_SIZE):
        pose = torch.from_numpy((np.random.rand(POSE_SIZE) - 0.5) * POSE_STDDEV).type(torch.float64).to(device)
        # Set rotation parameters to 0
        pose[0:3] = np.pi/2
        betas = torch.from_numpy((np.random.rand(BETA_SIZE) - 0.5) * BETAS_STDDEV).type(torch.float64).to(device)
...
if __name__ == '__main__':
    ...
    generate_smpl(opt, 0, [1])  # male
    generate_smpl(opt, 1, [1])  # female
