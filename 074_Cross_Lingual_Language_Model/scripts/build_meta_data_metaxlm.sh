...
if [ $MONO = "True" ] && [ -d $MONO_PATH ]; then
    for pair in $(echo $sub_tasks | sed -e 's/\,/ /g'); do
        for lg in $(echo $pair | sed -e 's/\-/ /g'); do
            if [ ! -f $MONO_PATH/$pair/$lg.all ]; then
                if [ $N_SAMPLES = "False" ];then
                    cat $MONO_PATH/$pair/$lg.txt | $TOKENIZE $lg $lg $threads_for_tokenizer | python $LOWER_REMOVE_ACCENT > $MONO_PATH/$pair/$lg.all
                else
                    get_n_samples $MONO_PATH/$pair/$lg.txt $N_SAMPLES $MONO_PATH/$pair/samples.$lg
                    cat $MONO_PATH/$pair/samples.$lg | $TOKENIZE $lg $threads_for_tokenizer | python $LOWER_REMOVE_ACCENT > $MONO_PATH/$pair/$lg.all 
                    # todo : memory
                    rm $MONO_PATH/$pair/samples.$lg
                fi
                echo "*** Tokenized (+ lowercase + accent-removal) $lg data to $MONO_PATH/$pair/? ***"
            else
                #rm $PARA_PATH/$pair.$lg.all
                echo "file $MONO_PATH/$pair/$lg.all already exists" 
            fi
        done
    done
fi
...
if [ $MONO = "True" ] && [ -d $MONO_PATH ]; then
    for pair in $(echo $sub_tasks | sed -e 's/\,/ /g'); do
        for lg in $(echo $pair | sed -e 's/\-/ /g'); do
            split_data $MONO_PATH/$pair/$lg.all $MONO_PATH/$pair/$lg.train $MONO_PATH/$pair/$lg.valid $MONO_PATH/$pair/$lg.test $test_size $val_size
        done
    done
fi
...
if [ ! -f $OUTPATH/bpe.train ]; then
    ...
    if [ $MONO = "True" ] && [ -d $MONO_PATH ]; then
        for pair in $(echo $sub_tasks | sed -e 's/\,/ /g'); do
            for lg in $(echo $pair | sed -e 's/\-/ /g'); do
                shuf -r -n $shuf_n_samples $MONO_PATH/$pair/$lg.train >> $OUTPATH/bpe.train
            done
        done
    fi
else
    ...
    for pair in $(echo $sub_tasks | sed -e 's/\,/ /g'); do
        for lg in $(echo $pair | sed -e 's/\-/ /g'); do
            for split in train valid test; do
                if [ ! -f $OUTPATH/$pair/$pair.$lg.$split ]; then
                    $FASTBPE applybpe $OUTPATH/$pair/$pair.$lg.$split $PARA_PATH/$pair/$pair.$lg.$split $OUTPATH/codes
                else
                    echo "file $OUTPATH/$pair/$pair.$lg.$split already exists"
                fi
            done
        done
    done
...
if [ $MONO = "True" ] && [ -d $MONO_PATH ]; then
    for pair in $(echo $sub_tasks | sed -e 's/\,/ /g'); do
        for lg in $(echo $pair | sed -e 's/\-/ /g'); do
            for split in train valid test; do
                if [ ! -f $OUTPATH/$pair/$split.$lg ]; then
                    $FASTBPE applybpe $OUTPATH/$pair/$split.$lg $MONO_PATH/$pair/$lg.$split $OUTPATH/codes
                      ...
                      if [ $PARA = "True" ]; then
                          for lg_tmp in $(echo $pair | sed -e 's/\-/ /g'); do
                              for split_tmp in train valid test; do
                                  ...
                                  cat $OUTPATH/$pair/$pair.$lg_tmp.$split_tmp >> $OUTPATH/$pair/$split.$lg
                              done
                          done
                      fi
                    fi
                else
                    echo "file $OUTPATH/$pair/$split.$lg already exists"
                fi
            done
        done
    done
...
build_fine_tune_data() {
    ...
    for (( i=0; i<${#array1[*]}; ++i)); do 
        ...
        if [ $data_percent != "" ] && [ $data_percent -gt 0 ] ;then
            pair=${array1[$i]}
            for lg in $(echo $pair | sed -e 's/\-/ /g'); do
                if [ ! -d $OUTPATH/$pair/fine_tune ]; then
                    mkdir $OUTPATH/$pair/fine_tune
                else
                    echo "dir $OUTPATH/$pair/fine_tune already exists"
                fi
                for split in train valid test; do
                    # PARA
                    if [ $PARA = "True" ]; then
                        name=$OUTPATH/$pair/$pair.$lg.$split
                        NLINES=`wc -l $name`;
                        ...
                        if [ $NLINES -le $N_FINE_TUNE ]; then
                            ...
                            mv $OUTPATH/$pair/$pair.$lg.$split $OUTPATH/$pair/$pair.$lg.$split.tmp
                            shuf --random-source=<(get_seeded_random 42) $OUTPATH/$pair/$pair.$lg.$split.tmp | head -$NREST > $OUTPATH/$pair/$pair.$lg.$split;
                            shuf --random-source=<(get_seeded_random 42) $OUTPATH/$pair/$pair.$lg.$split.tmp | tail -$N_FINE_TUNE > $OUTPATH/$pair/fine_tune/$pair.$lg.$split;
                            # todo : memory
                            rm $OUTPATH/$pair/$pair.$lg.$split.tmp
                        fi
                    fi

                    # MONO
                    if [ $MONO = "True" ] && [ -d $MONO_PATH ]; then
                        name=$OUTPATH/$pair/$split.$lg
                        ...
                        if [ $NLINES -le $N_FINE_TUNE ]; then
                            ...
                            NREST=$(($NLINES - $N_FINE_TUNE));
                            mv $OUTPATH/$pair/$split.$lg $OUTPATH/$pair/$split.$lg.tmp
                            shuf --random-source=<(get_seeded_random 42) $OUTPATH/$pair/$split.$lg.tmp | head -$NREST > $OUTPATH/$split.$lg; 
                            shuf --random-source=<(get_seeded_random 42) $OUTPATH/$pair/$split.$lg.tmp | tail -$N_FINE_TUNE > $OUTPATH/$pair/fine_tune/$split.$lg;
                            # todo : memory
                            rm $OUTPATH/$pair/$split.$lg.tmp
                        fi
                    fi
                done
            done
...
if [ $PARA = "True" ]; then
    for pair in $(echo $sub_tasks | sed -e 's/\,/ /g'); do
        for lg in $(echo $pair | sed -e 's/\-/ /g'); do
            for split in train valid test; do
                if [ ! -f $OUTPATH/$pair/$pair.$lg.$split.pth ]; then
                    python preprocess.py $OUTPATH/vocab $OUTPATH/$pair/$pair.$lg.$split
                else
                    echo "file $OUTPATH/$pair/$pair.$lg.$split.pth already exists"
                fi
                if [ -f $OUTPATH/$pair/fine_tune/$pair.$lg.$split ]; then
                    if [ ! -f $OUTPATH/$pair/fine_tune/$pair.$lg.$split.pth ]; then
                        python preprocess.py $OUTPATH/vocab $OUTPATH/$pair/fine_tune/$pair.$lg.$split
                    else
                        echo "file $OUTPATH/$pair/fine_tune/$pair.$lg.$split.pth already exists"
                    fi
                fi
            done
        done
        ...
        for lg in $(echo $pair | sed -e 's/\-/ /g'); do
            for split in train valid test; do
                if [ ! -f $OUTPATH/$pair/$split.$lg.pth ]; then
                    python preprocess.py $OUTPATH/vocab $OUTPATH/$pair/$split.$lg
                else
                    echo "file $OUTPATH/$pair/$split.$lg.pth already exists"
                fi
                if [ -f $OUTPATH/$pair/fine_tune/$split.$lg ]; then
                    if [ ! -f $OUTPATH/$pair/fine_tune/$split.$lg.pth ]; then
                        python preprocess.py $OUTPATH/vocab $OUTPATH/$pair/fine_tune/$split.$lg
                    else
                        echo "file $OUTPATH/$pair/fine_tune/$split.$lg.pth already exists"
                    fi
                fi
            done
        done
    ...
    for pair in $(echo $sub_tasks | sed -e 's/\,/ /g'); do
        for lg in $(echo $pair | sed -e 's/\-/ /g'); do
            for split in train valid test; do
                ...
                if [ -f $OUTPATH/$pair/fine_tune/$pair.$lg.$split.pth ]; then
                    if [ ! -f $OUTPATH/$pair/fine_tune/$split.$lg.pth ]; then
                        cp $OUTPATH/$pair/fine_tune/$pair.$lg.$split.pth $OUTPATH/$pair/fine_tune/$split.$lg.pth
                    else
                        echo "file $OUTPATH/$pair/fine_tune/$split.$lg.pth already exists"
                    fi
                fi  
...
for pair in $(echo $sub_tasks | sed -e 's/\,/ /g'); do
    for lg in $(echo $pair | sed -e 's/\-/ /g'); do
        for split in train valid test; do
            if [ ! -f $OUTPATH/$pair/$split.$pair.$lg.pth ]; then
                cp $OUTPATH/$pair/$pair.$lg.$split.pth $OUTPATH/$pair/$split.$pair.$lg.pth
            else
                echo "file $OUTPATH/$pair/$split.$pair.$lg.pth already exists"
            fi
            if [ -f $OUTPATH/$pair/fine_tune/$pair.$lg.$split.pth ]; then
                if [ ! -f $OUTPATH/$pair/fine_tune/$split.$pair.$lg.pth ]; then
                    cp $OUTPATH/$pair/fine_tune/$pair.$lg.$split.pth $OUTPATH/$pair/fine_tune/$split.$pair.$lg.pth
                else
                    echo "file $OUTPATH/$pair/fine_tune/$split.$pair.$lg.pth already exists"
                fi
            fi    
        done
    done
