...
from face_landmarks import MarkDetector
import os
import sys
...
def read_image_file(img_folder): 
    ...
    for image in tqdm(images): 
        ...
        marks = detect_land_marks(img.copy())
...
def find_pose_detection_for_landmarks(img, draw_boxes = True): 
...
def read_image_file(img_folder): 
    ...
    for image in tqdm(images): 
        img = cv2.imread(image)
        poses = find_pose_detection_for_landmarks(img.copy(), False)
        
        #cv2.imwrite("./LandmarkDetected/mtcnn_" + str(image.split("\\")[-1]), img4)
        # cv2.imwrite("./LandmarkDetected/dlib_" + image.split("\\")[-1].split(".jpg")[0], img2)
        #cv2.imwrite("./LandmarkDetected/haar_" + str(image.split("\\")[-1]), img3)
        cv2.imwrite("./HeadPosesDetected/dnn_headpose_" + str(image.split("\\")[-1]), poses)
