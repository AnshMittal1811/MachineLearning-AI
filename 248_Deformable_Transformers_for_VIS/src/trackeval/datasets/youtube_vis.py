...
class YouTubeVIS(_BaseDataset):
    ...
    def __init__(self, config=None, gt_name_file='youtube_vis_', tracker_name_file='youtube_vis_', gt=None, predictions=None):
        """Initialise dataset, checking that all required files are present"""
        super().__init__()
        # Fill non-given config values with defaults
        self.config = utils.init_config(config, self.get_default_dataset_config(), self.get_name())
        self.iou_type = ""
        self.gt_fol = self.config['GT_FOLDER'] + gt_name_file + self.config['SPLIT_TO_EVAL']
        self.tracker_fol = self.config['TRACKERS_FOLDER'] +  tracker_name_file + self.config['SPLIT_TO_EVAL']
        ...
        elif isinstance(gt, str) or gt is None:
            if gt is None:
                ...
                gt_file = os.path.join(self.gt_fol, gt_dir_files[0])
            else:
                if not os.path.exists(gt):
                    raise TrackEvalException(f"Given gt_file path does not exist {gt}")
                gt_file = gt

            with open(gt_file) as f:
                self.gt_data = json.load(f)
        else:
            raise TrackEvalException("Given gt is not a list or a file_path")
        ...
        if predictions is None:
            ...
            if len(self.tracker_list) != 1:
                raise TrackEvalException("Only 1 tracker can be evaluated when passing predictions")
            self.tracker_data[self.tracker_list[0]] = predictions
    ...
    def _prepare_gt_annotations(self):
        ...
        for track in self.gt_data['annotations']:
            ...
            for i, seg in enumerate(track['segmentations']):
                if seg and isinstance(seg['counts'], list):
