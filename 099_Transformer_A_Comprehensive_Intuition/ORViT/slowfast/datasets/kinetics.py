...
class Kinetics(torch.utils.data.Dataset):
    ...
    def _construct_loader(self):
        ...
        BAD_VIDS = set()
        with open(os.path.join(self.cfg.DATA.PATH_TO_DATA_DIR, 'bad_videos.txt'), 'rt') as f: d = f.read()
        for l in d.split('\n'): BAD_VIDS.add(l)
        ...
        with pathmgr.open(path_to_file, "r") as f:
            for clip_idx, path_label in enumerate(f.read().splitlines()):
                ...
                assert (
                    ...
                    == 2
                ), [path_label, path_label.split(self.cfg.DATA.PATH_LABEL_SEPARATOR)]
                path, label = path_label.split(
                    self.cfg.DATA.PATH_LABEL_SEPARATOR
                )
                path_to_vid = os.path.join(self.cfg.DATA.PATH_PREFIX, path)
                if path_to_vid in BAD_VIDS:
                    n_bad_vids += 1
                    continue
                for idx in range(self._num_clips):
                    self._path_to_videos.append(path_to_vid)
                    self._labels.append(int(label))
                    self._spatial_temporal_idx.append(idx)
                    self._video_meta[clip_idx * self._num_clips + idx] = {}
        logger.warning(f"Filtered {n_bad_vids} (known) bad videos")
        assert (
    ...
    def __getitem__(self, index):
        ...
        while True:
            ...
            try:
                ...
                print(f"Num of bad vids: {len(self.bad_vids)}")
            idx = random.randint(0, len(self) - 1)
    def getitem(self, index):
        ...
        for i_try in range(self._num_retries):
            ...
            self.bad_vids.add(self._path_to_videos[index])
            raise RuntimeError(
