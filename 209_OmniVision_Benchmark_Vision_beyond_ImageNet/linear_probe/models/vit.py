...
def load_clip_state_vision_model(model, ckpt_path):
    ...
    if ckpt_state:
        logger.info('==> Loading vision model state from CLIPVIT model..')
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, clip_pretrain_path: str):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        ...
        if clip_pretrain_path is not None:
            load_clip_state_vision_model(self, clip_pretrain_path)
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = torch.cat([self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype).cuda(), x], dim=1)  # shape = [*, grid ** 2 + 1, width]
...
def clipvitb16(**kwargs):
    ...
    default_kwargs = {
        ...
        'patch_size': 16,  # keep same with text transformer
        'width': 768,
        'layers': 12,
        'heads': 768 // 64,
        'output_dim': 512
...
def clipvitb32(**kwargs):
    ...
    default_kwargs = {
        'input_resolution': 224,
        'patch_size': 32,  # keep same with text transformer
        'width': 768,
        'layers': 12,
        'heads': 768 // 64,
        'output_dim': 512
