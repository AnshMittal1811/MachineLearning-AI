...
class IDRRayTracing(nn.Module):
    ...
    def forward(self,
        ...
        if mask_left_out.sum() > 0:  # project the origin to the not intersect points on the sphere
            ...
            acc_start_dis[mask_left_out] = - \
                torch.bmm(rays_left_out.view(-1, 1, 3), cam_left_out.view(-1, 3, 1)).squeeze()
    ...
    def sphere_tracing(self, batch_size, num_pixels, sdf, cam_loc, ray_directions, mask_intersect, sphere_intersections):
        ...
        sphere_intersections_points = cam_loc.reshape(
            batch_size, cam_loc.shape[1], 1, cam_loc.shape[-1]) + sphere_intersections.unsqueeze(-1) * ray_directions.unsqueeze(2)
        ...
        while True:
            ...
            # Update points
            curr_start_points = (cam_loc + acc_start_dis.reshape(batch_size,
                                                                 num_pixels, 1) * ray_directions).reshape(-1, 3)
            curr_end_points = (cam_loc + acc_end_dis.reshape(batch_size,
                                                             num_pixels, 1) * ray_directions).reshape(-1, 3)
            ...
            while (not_projected_start.sum() > 0 or not_projected_end.sum() > 0) and not_proj_iters < self.line_step_iters:
                ...
                curr_start_points[not_projected_start] = (
                    cam_loc + acc_start_dis.reshape(batch_size, num_pixels, 1) * ray_directions).reshape(-1, 3)[not_projected_start]
    ...
    def ray_sampler(self, sdf, cam_loc, object_mask, ray_directions, sampler_min_max, sampler_mask):
        ...
        points = cam_loc.reshape(batch_size, cam_loc.shape[1], 1, cam_loc.shape[-1]) + \
            pts_intervals.unsqueeze(-1) * ray_directions.unsqueeze(2)
        ...
        tmp = torch.sign(sdf_val) * torch.arange(self.n_steps, 0, -1).cuda().float().reshape((1,
                                                                                              self.n_steps))  # Force argmin to return the first min value
