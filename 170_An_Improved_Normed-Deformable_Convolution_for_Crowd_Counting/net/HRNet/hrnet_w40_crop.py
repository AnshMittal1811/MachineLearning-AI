...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, leaky_relu=False, leaky_scale=0, **kwargs):
      ...
      #      self.stage4_cfg, num_channels, multi_scale_output=True)
        ...
        last_inp_channels = np.int(np.sum(pre_stage_channels)) + 256 + 40
        ...
        # use out of space leaky relu here
        self.final_act = nn.ReLU(True) if not leaky_relu else nn.LeakyReLU(leaky_scale, False)
    ...
    #    x3 = F.upsample(x[3], size=(x0_h, x0_w), mode='bilinear', align_corners=False)
        ...
        x = torch.cat([x[0], x1, x2, x_head_1, x_head_2], 1)
