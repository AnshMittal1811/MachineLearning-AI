...
from lvvit.data import create_token_label_target, TokenLabelMixup, FastCollateTokenLabelMixup, create_token_label_loader, create_token_label_dataset
...
parser.add_argument('--eval-metric', default='224_1_acc1', type=str, metavar='EVAL_METRIC',
...
def main():
    ...
    if args.eval:
        ...
        eval_metrics = validate(model_ema.module, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast)
        print(eval_metrics)
        return   
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        for i in [8,10,12,14]:
            scale = 14/i
            bbox.append((int(bbx1//scale), int(bby1//scale), int(bbx2//scale), int(bby2//scale)))
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            for sub_network in sub_networks:
                with amp_autocast():
                    output = model(input, sub_network[0], sub_network[1],)
        ...
        if args.local_rank == 0:
            ...
            for sub_network in sub_networks: 
                _logger.info('* {image_size}_{keep_ratio}_Acc@1 {top1.global_avg:.3f} {image_size}_{keep_ratio}_Acc@5 {top5.global_avg:.3f} '
                    .format(log_name,image_size=sub_network[0], keep_ratio=sub_network[1], top1=metric_logger.meters[f'{sub_network[0]}_{sub_network[1]}_acc1'], top5=metric_logger.meters[f'{sub_network[0]}_{sub_network[1]}_acc5']))
