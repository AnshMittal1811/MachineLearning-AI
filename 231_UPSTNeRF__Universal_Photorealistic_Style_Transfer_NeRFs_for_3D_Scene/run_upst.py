...
from lib import utils, upst, upstmpi
...
def get_style_input(style_imgs, mode=None, test_seed=None, style_ID=None):
    if mode == "test":
        random.seed(test_seed)
        if style_ID != None:
            style_img_path = style_imgs[style_ID]
        else:
            style_img_path = random.sample(style_imgs, 1)[0]
    else:
        style_img_path = np.random.choice(style_imgs, 1)[0]
    
    ori_style_img = Image.open(style_img_path).convert('RGB')
    style_img = data_transform(ori_style_img)
    style_idx = torch.from_numpy(np.array([style_imgs.index(style_img_path)]))
...
def config_parser():
    ...
    # testing options
    parser.add_argument("--style_base_root",default="./data/coco/mscoco2017")
...
def scene_rep_reconstruction_style(args, cfg, cfg_model, cfg_train, data_dict, stage, coarse_ckpt_path=None):

    style_base_root = args.style_base_root
    style_dir = os.path.join(style_base_root, "train2017")
    style_img_files = find_files(style_dir, exts=['*.png', '*.jpg'])

    style_dir = os.path.join(style_base_root, "test2017")
    style_img_files+= find_files(style_dir, exts=['*.png', '*.jpg'])

    # logger.info("Number of style images is {}".format(len(style_img_files)))
    print("Number of style images is {}".format(len(style_img_files)))
    ...
    if reload_ckpt_path is None:
        print(f'scene_rep_reconstruction ({stage}): reload from {coarse_ckpt_path}')
        if cfg.data.ndc:
            model_class = upstmpi.UPSTMPI_DirectGO
        else:
            model_class = upst.UPST_DirectVoxGO
        model = utils.load_model(model_class, coarse_ckpt_path).to(device)
    ...
    viewdirs_tr=[]
    for (H, W), K, c2w in zip(HW[i_train], Ks[i_train], poses[i_train]):
        rays_o, rays_d, viewdirs = upst.get_rays_of_a_view(#得到所有的光线起点和角度,以及观测角度,(W,H,3)->([800, 800, 3])
                H, W, K, c2w, ndc=cfg.data.ndc, inverse_y=render_kwargs['inverse_y'],
                flip_x=cfg.data.flip_x, flip_y=cfg.data.flip_y)
    ...
    glow = YUVStyleNet(3)
    print(glow)
    print('Total params: %.2fM' % (sum(p.numel() for p in glow.parameters()) / 1000000.0))
    decoderdefault='./YUVStyleNet.pth'
    if os.path.isfile(decoderdefault):
        print("--------loading checkpoint----------")
        checkpoint = torch.load(decoderdefault)
        args.start_iter = checkpoint['iter']
        glow.load_state_dict(checkpoint['state_dict'])
        print("=> loaded checkpoint '{}'".format(decoderdefault))
    else:
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        random_i = np.random.randint(len(i_train))
        # random_i=0
        target_rgb = images[i_train][random_i]
        c2w = poses[random_i]

        rays_o = rays_o_tr[random_i].flatten(0,-2)
        rays_d = rays_d_tr[random_i].flatten(0,-2)
        viewdirs = viewdirs_tr[random_i].flatten(0,-2)

        total_inds = np.arange(H*W).reshape(H, W)
        patch_H, patch_W = 100, 100
        num_region_H, num_region_W = H//patch_H, W//patch_W
        ...
        rays_o_temp = rays_o[select_inds, :]
        rays_d_temp = rays_d[select_inds, :]
        viewdirs_temp = viewdirs[select_inds, :]
        target_rgb = target_rgb.flatten(0,-2)[select_inds, :]  
        ...
        if global_step < 0:
            style_embedded = None
            optimizer.zero_grad()
            render_result = model(rays_o_temp, rays_d_temp, viewdirs_temp,style_embedded, global_step=global_step, **render_kwargs)
            # gradient descent step
            target = target_rgb.flatten(0,-2)
            target = target_rgb.to(device)

            rgb_loss = cfg_train.weight_main * utils.img2mse(render_result['rgb_marched'], target)
            ...
            #1
            model.k0.requires_grad = False
            model.density.requires_grad = False
            style_img, style_idx= get_style_input(style_imgs=style_img_files, mode=None, test_seed=None, style_ID=None)
            style_embedded = style_img.cuda()
            target_rgb = target_rgb.to(device)
            # optimizer.zero_grad(set_to_none=True)
            optimizer.zero_grad()
            render_result = model(rays_o_temp, rays_d_temp, viewdirs_temp,torch.unsqueeze(style_embedded, 0), global_step=global_step, **render_kwargs)
            ...
            # # gradient descent step
            alphainv_last = render_result['alphainv_last'].reshape(patch_H,patch_W,1)
            alphainv_last = alphainv_last.repeat_interleave(3, dim=2)


            zero_temp = torch.zeros_like(alphainv_last)
            alphainv_last = torch.where(alphainv_last < 1.0, zero_temp, alphainv_last)


            style_img = torch.unsqueeze(style_img, 0)
            style_img = style_img.to(device)
            content_images = torch.unsqueeze(target_rgb.reshape(patch_H,patch_W,3).permute(2,0,1), 0)
            content_images = F.interpolate(content_images, (style_img.size()[2], style_img.size()[3]),  mode='bilinear', align_corners=False)


            s_output, stylized_target = glow(content_images,style_img)
            stylized_target_out = F.interpolate(stylized_target, (patch_H, patch_W), mode='bilinear', align_corners=False)
            stylized_target_out = torch.squeeze(stylized_target_out,0).permute(1,2,0)
            stylized_target_out = stylized_target_out.reshape(-1,3)


            alphainv_last_temp = alphainv_last.reshape(-1,3)
            one_temp = torch.ones_like(stylized_target_out)
            stylized_target_out = torch.where(alphainv_last_temp == 1.0, one_temp, stylized_target_out)


            target = stylized_target_out.flatten(0,-2)
            rgb_loss = cfg_train.weight_main * utils.img2mse(render_result['rgb_marched'], target)
            psnr = utils.mse2psnr(rgb_loss.detach())
            loss = rgb_loss
        ...
        if global_step%args.i_print==0:
            ...
            tqdm.write(f'scene_rep_reconstruction ({stage}): iter {global_step:6d} / '
                       f'Loss: {losses.avg:.9f} / PSNR: {np.mean(psnr_lst):5.2f} / '
                       f'c_losses: {c_losses.avg:.6f} / '
                       f's_losses: {s_losses.avg:.6f} / '
                       f'lr: {lr_temp:.6f} / '
                       f'Eps: {eps_time_str}')
            psnr_lst = []
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    if reload_ckpt_path is None:
        ...
        if cfg.data.ndc:
            ...
            model = upstmpi.UPSTMPI_DirectGO(
    ...
    def gather_training_rays():
        if data_dict['irregular_shape']:
            ...
            if cfg_train.ray_sampler == 'in_maskcache':
                rgb_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz = upst.get_training_rays_in_maskcache_sampling(
                ...
                rgb_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz = upst.get_training_rays_flatten(
                    rgb_tr_ori=rgb_tr_ori,
                    train_poses=poses[i_train],
                    HW=HW[i_train], Ks=Ks[i_train], ndc=cfg.data.ndc, inverse_y=cfg.data.inverse_y,
                    flip_x=cfg.data.flip_x, flip_y=cfg.data.flip_y)
            else:
                rgb_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz = upst.get_training_rays_irregular_shape(
                    rgb_tr=rgb_tr_ori,
                    train_poses=poses[i_train],
                    HW=HW[i_train], Ks=Ks[i_train], ndc=cfg.data.ndc, inverse_y=cfg.data.inverse_y,
                    flip_x=cfg.data.flip_x, flip_y=cfg.data.flip_y)
            ...
            if cfg_train.ray_sampler == 'in_maskcache':
                ...
                rgb_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz = upst.get_training_rays(
                    rgb_tr=rgb_tr_ori,
                    train_poses=poses[i_train],
                    HW=HW[i_train], Ks=Ks[i_train], ndc=cfg.data.ndc, inverse_y=cfg.data.inverse_y,
                    flip_x=cfg.data.flip_x, flip_y=cfg.data.flip_y)

        index_generator = upst.batch_indices_generator(len(rgb_tr), cfg_train.N_rand)
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        if global_step in cfg_train.pg_scale:
            ...
            if isinstance(model, upst.UPST_DirectVoxGO):
                model.scale_volume_grid(cur_voxels)
            elif isinstance(model, upstmpi.UPSTMPI_DirectGO):
        ...
        # volume rendering
        # style_embedded = style_images
        style_embedded = None
        # render_result = model(rays_o, rays_d, viewdirs,torch.unsqueeze(style_embedded, 0), global_step=global_step, **render_kwargs)
        render_result = model(rays_o, rays_d, viewdirs,style_embedded, global_step=global_step, **render_kwargs)
        # gradient descent step
        optimizer.zero_grad(set_to_none=True)
...
def train(args, cfg, data_dict):
    ...
    if cfg.data.ndc:
        ...
        xyz_min_fine, xyz_max_fine = compute_bbox_by_coarse_geo(
                model_class=upst.UPST_DirectVoxGO, model_path=coarse_ckpt_path,
    ...
    # Style transfer and reconstruction
    coarse_ckpt_path = os.path.join(cfg.basedir, cfg.expname, f'fine_last.tar')
    eps_fine_style = time.time()
    scene_rep_reconstruction_style(
            args=args, cfg=cfg,
            cfg_model=cfg.fine_model_and_render, cfg_train=cfg.style_train,
            data_dict=data_dict, stage='fine_style',
            coarse_ckpt_path=coarse_ckpt_path)

    eps_fine_style = time.time() - eps_fine_style
    eps_time_str = f'{eps_fine_style//3600:02.0f}:{eps_fine_style//60%60:02.0f}:{eps_fine_style%60:02.0f}'
    print('train: style reconstruction in', eps_time_str)
...
if __name__=='__main__':
    ...
    if args.export_coarse_only:
        ...
        with torch.no_grad():
            ...
            model = utils.load_model(upst.UPST_DirectVoxGO, ckpt_path).to(device)
