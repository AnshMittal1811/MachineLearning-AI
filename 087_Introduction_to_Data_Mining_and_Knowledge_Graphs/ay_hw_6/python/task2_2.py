...
def split2NChunks(list_data, num_of_chunk):
    size = int(math.ceil(len(list_data) / float(num_of_chunk)))
...
def applyFM(rdd):
    ...
    avg_list = sorted(map(lambda val: sum(val) / max(len(val), 1), clusters))
...
if __name__ == '__main__':
    ...
    # port = int('9999')
    # output_file_path = "../out/task2_3.csv"
    # flag = True
