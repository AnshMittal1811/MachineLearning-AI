...
class AugmentOp:
    ...
    def __call__(self, img_list, boxes=None):
        if self.seed is not None:
            np.random.seed(self.seed)
            random.seed(self.seed)
        if self.prob < 1.0 and random.random() > self.prob:
            return img_list if boxes is None else (img_list, boxes)
...
class RandAugment:
    ...
    def __call__(self, img, boxes=None):    
        ...
        for op in ops:
            img = op(img, boxes=boxes)
            if boxes is not None: img , boxes = img
        return img if boxes is None else (img, boxes)
