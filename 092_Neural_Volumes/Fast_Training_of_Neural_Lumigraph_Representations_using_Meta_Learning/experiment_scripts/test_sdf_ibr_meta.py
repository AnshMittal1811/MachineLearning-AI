"""
Trains the SDF+Color model using 3D or 2D multi-view data.
"""
...
def get_arg_parser():
    ...
    # Save/resume.
    p.add_argument('--logging_root', type=str, default='./logs_meta_spec_final', help='root for logging')
    ...
    # Dataset
    p.add_argument('--dataset_path', type=str, default='/home/sitzmann/data/',
                   help='Path to dataset folder.')
    p.add_argument('--dataset_type', type=str, default='sinesdf_static',
                   help='Dataset type [sinesdf_static|sinesdf_video].')
    ...
    # Initialization to begin from.
    p.add_argument('--meta_initialization', type=str, default=None,
                   help='Meta-initialization to begin training with')
    # Meta learning.
...
def main():
    ...
    # summary_fn = partial(utils_ibr.write_sdf_color_summary, opt, sdf_dataset)
    ...
    meta_model.load_checkpoint(opt.meta_initialization)
    model = meta_model.hypo_module
    ...
    training.train(model=model, train_dataloader=dataloader, epochs=opt.num_epochs, lr=opt.lr,
                   ...
                   teacher=None, meta_spec_log=True)
