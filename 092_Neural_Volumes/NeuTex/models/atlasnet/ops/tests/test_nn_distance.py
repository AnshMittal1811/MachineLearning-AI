...
from completion.ops.nn_distance import nn_distance
...
def nn_distance_torch(xyz1, xyz2, data_format='NWC'):
    ...
    if data_format == 'NCW':
        assert xyz1.size(1) in [2, 3] and xyz2.size(1) in [2, 3]
    elif data_format == 'NWC':
        assert xyz1.size(2) in [2, 3] and xyz2.size(2) in [2, 3]
...
def test():
    ...
    num_points = 2043
    dim = 3
    xyz1 = torch.rand(batch_size, dim, num_points).float()
    xyz2 = torch.rand(batch_size, dim, num_points).float()
    xyz1 = xyz1.cuda()
    xyz2 = xyz2.cuda()

    # check forward
    dist1_actual, idx1_actual = nn_distance(xyz1, xyz2, transpose=True)
    ...
    batch_size = 2
    num_points = 64
    xyz1 = torch.rand(batch_size, dim, num_points).double()
    xyz2 = torch.rand(batch_size, dim, num_points).double()
    xyz1 = xyz1.cuda()
    xyz2 = xyz2.cuda()
    xyz1.requires_grad = True
    xyz2.requires_grad = True
    func = lambda xyz1, xyz2: nn_distance(xyz1, xyz2, True)[0]
    gradcheck(func, (xyz1, xyz2))
