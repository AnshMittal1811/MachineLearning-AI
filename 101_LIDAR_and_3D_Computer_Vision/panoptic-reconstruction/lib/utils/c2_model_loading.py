...
def _rename_conv_weights_for_deformable_conv_layers(state_dict):
    import re
    logger = logging.getLogger(__name__)
    logger.info("Remapping conv weights for deformable conv weights")
    layer_keys = sorted(state_dict.keys())
    for ix, stage_with_dcn in enumerate(config.MODEL.RESNETS.STAGE_WITH_DCN, 1):
...
def load_resnet_c2_format(f):
    ...
    conv_body = config.MODEL.BACKBONE.CONV_BODY
    ...
    state_dict = _rename_conv_weights_for_deformable_conv_layers(state_dict)
    # ***********************************
    return dict(model=state_dict)


def load_c2_format(f):
    return C2_FORMAT_LOADER[config.MODEL.BACKBONE.CONV_BODY](f)
