...
from .boxes_autoaugment import rand_augment_transform as rand_augment_transform_with_boxes
...
def crop_clip_boxes(boxes, x_offset, y_offset, size):
    """
    ...
    if isinstance(boxes, np.ndarray):
        ...
        clipf=torch.clip
    cropped_boxes[..., [0, 2]] = clipf(boxes[..., [0, 2]] - x_offset,0,size)
    cropped_boxes[..., [1, 3]] = clipf(boxes[..., [1, 3]] - y_offset,0,size)
...
def random_crop(images, size, boxes=None):
    ...
    cropped_boxes = (
        crop_clip_boxes(boxes, x_offset, y_offset, size) if boxes is not None else None
    )
...
def random_resized_crop(
    ...
    if boxes is not None:
        ...
        boxes[..., [0, 2]] = np.clip(boxes[..., [0, 2]], 0, w) * float(target_width) / w
        boxes[..., [1, 3]] = np.clip(boxes[..., [1, 3]], 0, h) * float(target_height) / h
        return ret, boxes
    else:
        return ret
...
def create_random_augment(
    ...
    if auto_augment:
        ...
        if auto_augment.startswith("rand"):
            rand_aug_func = rand_augment_transform_with_boxes if with_boxes else rand_augment_transform
            return ComposeWithBoxes(
                [rand_aug_func(auto_augment, aa_params)]
            )
    raise NotImplementedError
