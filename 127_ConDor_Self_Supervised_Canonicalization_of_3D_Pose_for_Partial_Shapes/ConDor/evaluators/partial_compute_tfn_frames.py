...
from utils.data_prep_utils import tf_center
import math
...
def forward_pass(model, x, frame = False):
    ...
    out_dict = model(x, training = False)
...
def slice_direction(xij, dir_vec_instance, num_points_partial):

    batch_size = xij.shape[0]
    vec_batch = tf.tile(tf.expand_dims(dir_vec_instance, axis = 0), [xij.shape[0], 1])
    vx = tf.einsum('bj,bpj->bp', vec_batch, xij)
    ...
    xij_partial = tf.gather_nd(xij, x_partial_idx)
...
def save_frames(model, filename, shapes_src_path, rot_src_path, tar_path, slice_file, batch_size=32, num_rots=128, procrustes = 1, num_points_slice = None):
    
    fv = h5py.File(os.path.join(slice_file), 'r')
    fx = h5py.File(os.path.join(shapes_src_path, filename), 'r')
    print(tar_path, slice_file)
    x = fx['data'][:]
    dir_vector = fv["data"][:]
    fv.close()
    fx.close()
    num_shapes = x.shape[0]
    num_dir = dir_vector.shape[0]
    fr = h5py.File(os.path.join(rot_src_path))
    r = fr['data'][:]
    print(r.shape)
    print(x.shape)
    ...
    for i in tqdm(range(num_batches), desc = "procrustes=%d" % procrustes):
        #print(100.*i/num_batches)
        xi = x[i * batch_size:(i + 1) * batch_size, ...]
        ...
        for j in range(num_rots):
            ...
            if num_points_slice is not None:
                if num_points_slice == "random":
                      slices = [256, 512, 768]
                      num_points_partial = slices[np.random.randint(len(slices))]
                else:
                    num_points_partial = num_points_slice
            else:
                num_points_partial = num_points // 2
            #print(num_points_partial, " num points")
            # Partial frames
            dir_vec_instance = dir_vector[j, ...]
            xij_partial = tf_center(slice_direction(xi, dir_vec_instance, num_points_partial))
            ...
            if procrustes:
                #print("using procrustes")
                Ri_partial.append(orth_procrustes(xij, yij))
            else: 
                Ri_partial.append(tf.linalg.inv(frame))
        ...
        Ri_partial = tf.stack(Ri_partial, axis = 1)
        Ri = tf.stack(Ri, axis=1)

        R_p.append(np.array(Ri_partial, dtype=np.float64))
        R.append(np.asarray(Ri, dtype=np.float64))
    R = np.concatenate(R, axis=0)
    R = R[:num_shapes, ...]
    
    R_p = np.concatenate(R_p, axis=0)
    R_p = R_p[:num_shapes, ...]

    tar_path_full = os.path.join(tar_path, "full/")
    check_dir_create(tar_path_full)

    h5_fout = h5py.File(os.path.join(tar_path_full, filename), 'w')
    h5_fout.create_dataset(
        'data', data=R,
        compression='gzip', compression_opts=4,
        dtype='float32')
    ...
    tar_path_partial = os.path.join(tar_path, "partial/")
    check_dir_create(tar_path_partial)

    h5_fout = h5py.File(os.path.join(tar_path_partial, filename), 'w')
    h5_fout.create_dataset(
        'data', data=R_p,
        compression='gzip', compression_opts=4,
        dtype='float32')
...
def build_model_new(model_path, batch_size=32, num_points=1024, frames = 5):
    
    inputs = tf.keras.layers.Input(batch_shape=(batch_size, None, 3))
    alignnet = tf.keras.models.Model(inputs=inputs, outputs=TFN(num_frames=frames, num_capsules=10, num_classes=1)(inputs))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--slice_file", type = str, required = True)
    parser.add_argument("--slice_points", type=str, default = None)
    parser.add_argument("--model_frames", type=int, default = 5)


    args = parser.parse_args()
    ########################################################################
    
    model = build_model_new(args.model, batch_size=32, num_points=1024, frames = args.model_frames)
    num_points = args.slice_points
    
    if num_points is not None and num_points.isdigit():
    ...
    for i in range(len(AtlasNetClasses)):
        ...
        if args.pca:
            save_pca_frames_(AtlasNetClasses[i], args.path_ds, args.path_rot, path_out, slice_file = args.slice_file, batch_size=32, num_rots=args.num_rots)
        else:
            save_frames(model, AtlasNetClasses[i], args.path_ds, args.path_rot, path_out, slice_file = args.slice_file, batch_size=32, num_rots=args.num_rots, procrustes = args.procrustes, num_points_slice = num_points)
