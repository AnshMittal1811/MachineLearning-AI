from partial_canonicalization_metrics import *
...
if __name__=="__main__":
    ...
    parser.add_argument("--rot_p_can", type = str, required = True)
    ...
    for i in range(len(AtlasNetClasses)):
        ...
        a = class_consistency_metric(AtlasNetClasses[i], args.path_shape, args.path_rot_gt, args.slice_file, args.rot_can, r_partial_can_path = args.rot_p_can, shapes_idx_path=args.shape_idx, batch_size=32, n_iter = args.n_iter)
        print("consistency: ", a)
        ma += a
        b = equivariance_metric(AtlasNetClasses[i], args.path_shape, args.path_rot_gt, args.slice_file, args.rot_can,
                                r_partial_can_path = args.rot_p_can, idx_path=args.rot_idx, batch_size=32, n_iter = args.n_iter)
        print("equivariance: ", b)
        mb += b
        c = class_consistency_umetric(AtlasNetClasses[i], args.path_shape, args.path_rot_gt, args.slice_file, args.rot_can,
                                    r_partial_can_path = args.rot_p_can,
