...
class TrainerNOCS(Trainer):
    ...
    def forward_pass(self, data, val_step = False):
        ...
        x_input = data["full"]
        x_partial_input = data["partial"]
        idx_partial_to_full = data["idx_partial_to_full"]
        
        # Center and rotate full
        x = tf_center(x_input)
        x, R = tf_random_rotate(x, return_rotation = True)
        
        # Center and rotate partial
        x_partial = tf_center(x_partial_input)
        x_partial = tf.einsum('bij,bpj->bpi', R, x_partial_input)
        
        x_kd, idx_kd, idx_inv_kd = kdtree_indexing_(x)
        x_partial_kd, idx_partial_kd, idx_partial_inv_kd = kdtree_indexing_(x_partial)
        ...
        with tf.GradientTape() as tape:
            ...
            error_full = tf.reduce_mean(tf.reduce_mean(tf.abs(x_kd[:, None] - inv_combined), axis = -1), axis = -1)
            ...
            pp_caps = slice_idx_data(caps, idx_partial_to_full, idx_inv_kd, idx_partial_kd)
            pp_inv = tf_center(slice_idx_data(inv, idx_partial_to_full, idx_inv_kd, idx_partial_kd))
            x_restriction_not_centered = slice_idx_data(x_kd, idx_partial_to_full, idx_inv_kd, idx_partial_kd)
            ...
            centroid_partial_inv = compute_centroids(x_partial_kd, normalized_caps_partial)
            ...
            if not val_step:
                ...
                self.optimizer.apply_gradients((grad, var) for (grad, var) in zip(gradients, self.model.trainable_variables) if grad is not None)
    ...
    def test_pass(self, data):
        ...
        x_partial_input = tf_center(x_partial_input)
        x_partial_input = tf_random_rotate(x_partial_input)
        ...
        error_partial = tf.reduce_mean(tf.reduce_mean(tf.abs(x_partial_input[:, None] - p_inv_combined), axis = -1), axis = -1)
        ...
        # Compute centroids
        centroids_partial = compute_centroids(x_partial_input, normalized_caps_partial)
