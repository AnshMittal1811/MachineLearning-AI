...
class TrainerScale(Trainer):
    ...
    def forward_pass(self, x, val_step = False):
        ...
        x_restriction_not_centered = tf.gather_nd(x, x_partial_idx)
        x_restriction, center_x = tf_center(x_restriction_not_centered, return_center = True)
        ...
        with tf.GradientTape() as tape:
            ...
            if features.scale.use:
                
                use_partial = np.random.rand() < 0.5
                scale_factor = np.random.uniform(features.scale.args.factor_min, features.scale.args.factor_max, size = (x.shape[0], 1, 1))
                
                if use_partial:
                    out_dict_scale = self.model(x_partial_input_centered * scale_factor, training=training_bool)
                else:
                    out_dict_scale = self.model(x * scale_factor, training=training_bool)


                s_caps, s_inv, s_basis = out_dict_scale['caps'], out_dict_scale['points_inv'], out_dict_scale["basis_list"]
                scale = out_dict_scale["scale"]
                scale_loss = tf.reduce_mean(tf.abs(scale - scale_factor))
            ...
            loss_dict.update({"loss": loss, "l2_loss": l2_loss_f, 
