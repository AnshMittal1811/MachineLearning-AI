    ...
    def forward_pass(self, x, val_step = False):
        ...
        with tf.GradientTape() as tape:
            ...
            if self.features.scale.use:
                ...
                error_full_scaled = tf.reduce_mean(tf.reduce_mean(tf.abs(x[:, None] - s_inv_combined), axis = -1), axis = -1)
                ...
                centroids_s = compute_centroids(x, normalized_caps_s)
                ...
                l2_loss_s, mean_root_square_s = compute_l2_loss(x, y_s)
                l2_loss_f += l2_loss_s
                mean_root_square_loss_f += mean_root_square_s
                chamfer_loss_f += chamfer_distance_l2(y_s, x) 
                hausdorff_loss_f += hausdorff_distance_l2(y_s, x)
                eq_loss += equilibrium_loss(s_caps)
                loc_loss += localization_loss_new(x, s_caps, centroids_s)
                caps_chamf_loss += chamfer_distance_l2(x, centroids_s)
                ...
                caps_scaled_l2_loss, _ = compute_l2_loss(tf.stop_gradient(inv), s_inv)     
