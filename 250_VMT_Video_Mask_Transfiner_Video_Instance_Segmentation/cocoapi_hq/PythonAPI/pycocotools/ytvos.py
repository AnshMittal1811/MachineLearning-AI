...
from .boundary_utils import augment_annotations_with_boundary_multi_core
...
class YTVOS:
    ...
    def loadRes(self, resFile):
        ...
        """
        res = YTVOS(get_boundary=self.get_boundary)
    ...
    def annToRLE(self, ann, frameId):
        ...
        if 'segmentations_refined' in ann.keys():
            segm = ann['segmentations_refined'][frameId]
        else:
    ...
    def annToRLEVideo(self, ann):
        """
        Convert annotation which can be polygons, uncompressed RLE to RLE.
        :return: binary mask (numpy 2D array)
        """
        # print('self.vids:', self.vids.keys())
        t = self.vids[ann['video_id']]
        h, w = t['height'], t['width']
        # print('len ann segm:', len(ann['segmentations_refined']))
        rle_list = []
        if 'segmentations_refined' in ann.keys():
            for frameId in range(len(ann['segmentations_refined'])):
                segm = ann['segmentations_refined'][frameId]
                # print('segm keys:', segm.keys())
                if segm is None:
            ...
            for frameId in range(len(ann['segmentations'])):
                segm = ann['segmentations'][frameId]
                # print('segm keys:', segm.keys())
                if segm is None:
                    rle_list.append(None)
    ...
    def annToMaskVideo(self, ann):
        """
        Convert annotation which can be polygons, uncompressed RLE, or RLE to binary mask.
        :return: binary mask (numpy 2D array)
        """
        # print('ann to video:', ann.keys())
        rle_list = self.annToRLEVideo(ann)
        m_list = []
        for rle in rle_list:
            if rle is None:
                m = np.zeros((ann['height'], ann['width']))
            else:
                m = maskUtils.decode(rle)
            m_list.append(m)
        return m_list
