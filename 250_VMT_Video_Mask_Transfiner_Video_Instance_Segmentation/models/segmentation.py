...
class VMT(nn.Module):
    def __init__(self, detr, rel_coord=True, freeze_detr=False):
        ...
        self.lay_hr1 = torch.nn.Conv2d(256, 256, 3, padding=1)
        self.lay1_h = torch.nn.Conv2d(256, 64, 3, padding=1)
    ...
    def inference(self, samples: NestedTensor,orig_w,orig_h):
        ...
        bottom_lvl_feature = features[0].decompose()[0].detach().clone().reshape(n//self.detr.num_frames, self.detr.num_frames, c, h_b, w_b)

        c_r, h_b_r, w_b_r = feature_hr[0].decompose()[0].shape[-3:]
        bottom_lvl_feature_hr = feature_hr[0].decompose()[0].detach().reshape(n//self.detr.num_frames, self.detr.num_frames, c_r, h_b_r, w_b_r)
