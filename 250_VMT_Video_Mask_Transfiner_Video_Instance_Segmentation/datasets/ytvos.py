...
class YTVOSDataset:
    ...
    def __getitem__(self, idx):
        ...
        while not instance_check:          
            ...
            target_inds = inds 
            target = self.prepare(img[0], target, target_inds, img_path, sample_inds = sample_indx)
...
def convert_coco_poly_to_mask(segmentations, height, width, is_crowd):
    ...
    for i, seg in enumerate(segmentations):
        if not seg:
            ...
            if not is_crowd[i]:
                if type(seg['counts']) is not list:
...
class ConvertCocoPolysToMask(object):
    ...
    def __call__(self, image, target, target_inds, image_path, sample_inds):
        w, h = image.size
        ...
        for i, ann in enumerate(anno):
            ...
            for id in target_inds:
                ...
                segm = ann['segmentations_refined'][sample_inds[id]]
        ...
        # ASCII_values = []
        # for character in image_path:
        #     ASCII_values.append(ord(character))
        # target['path'] = torch.tensor(ASCII_values)
...
def build(image_set, args):
    ...
    if args.dataset_file == 'YoutubeVIS' or args.dataset_file == 'jointcoco':
        mode = 'instances'
        PATHS = {
             "train": (root / "train/JPEGImages", root / "annotations" / "ytvis_hq-train.json"),
             "val": (root / "valid/JPEGImages", root / "annotations" / "ytvis_hq-val.json"),
        }
        img_folder, ann_file = PATHS[image_set]
        print('use HQ-YTVIS dataset')
