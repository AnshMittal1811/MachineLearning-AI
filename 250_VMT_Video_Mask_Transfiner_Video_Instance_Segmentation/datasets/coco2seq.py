...
class CocoDetection(TvCocoDetection):
    ...
    def __getitem__(self, idx):
        ...
        while not instance_check:   
            ...
            img, target = self.prepare(img, target, path)         
...
def convert_coco_poly_to_mask(segmentations, height, width):
    ...
    for polygons in segmentations:
        ...
        if type(polygons) is dict and type(polygons['counts']) is not list:
        ...
        # print('mask-coco2seq shape:', mask.shape)
        if len(mask.shape) < 3:
            mask = mask[..., None]
...
class ConvertCocoPolysToMask(object):
    ...
    def __call__(self, image, target, image_path):
        w, h = image.size
        ...
        if self.return_masks:
            # print('enter-2 coco2seq')
            # print('anno[0] keys:', anno[0].keys())
...
def build(image_set, args):
    ...
    if args.dataset_file == 'coco' or args.dataset_file == 'Seq_coco' or args.dataset_file == 'jointcoco':
        PATHS = {
            ...
            "train": (root / "train2017", './datasets/coco_keepfor_ytvis19_new.json'),
