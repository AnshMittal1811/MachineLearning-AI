...
model.load_weights("late_fusion_unimodal_95.hdf5",by_name=True)
...
for n in range(len(names)):
	print '=================image - '+str(n)+'==================='
	name = names[n].strip('\n')
	data[0] =cv2.resize(cv2.imread('/home/krishna/freiburg_forest_dataset/test/rgb/'+name+'.jpg'), input_size)
	a = model.predict_on_batch( data[0] )
	...
	cv2.imwrite('/home/krishna/freiburg_forest_dataset/test/GT_color/'+name+'_predicted_unimodal_AUG'+'.jpg', construct_label(a[0]))
	print 'Image saved at: '+'freiburg_forest_dataset/test/GT_color/'+name+'_predicted_unimodal_AUG'+'.jpg'
