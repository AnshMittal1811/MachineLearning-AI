...
class AttModel(CaptionModel):
    ...
    def forward(self, fc_feats, att_feats, seq):
        ...
        # TODO Probably need to change this to go upto maxlength or when 0 (EOS) is output
        # Check what it does when it evaluates and doesn't have seq
    ...
    def sample(self, fc_feats, att_feats, opt={}, filter_zerolength=False):
        sample_max = opt.get('sampling_type', 'argmax') == 'argmax'
        beam_size = opt.get('beam_size', 1)
        use_gumbel = opt.get('use_gumbel', 0) == 1
        temperature = opt.get('temperature', 0.5)
        if beam_size > 1:
        ...
        for t in range(self.seq_length + 1):
            if t == 0: # input <bos>
                ...
                _, it = torch.max(probs.data, 1)
                it = it.view(-1).long()
            else:
                if temperature == 1.0:
                    prob_prev = probs.data.cpu() # fetch prev distribution: shape Nx(M+1)
                else:
                    # scale probs by temperature
                    prob_prev = torch.div(probs.data, temperature).cpu()
            ...
            if t >= 1:
                ...
                allProbs.append(probs) # add the probs that were used to sample 'it'

            output, state = self.core(xt, fc_feats, att_feats, p_att_feats, state)
            if use_gumbel:
                logits = self.logit(output) + sample_gumbel(self.logits.size())
                probs = self.softmax(logits / temperature)
            else:
                probs = self.softmax(self.logit(output))
        ...
        allProbs = torch.cat([_.unsqueeze(1) for _ in allProbs], 1)
        max_length = len(seq[0])
        for i in range(len(seq)):
            # Get the indices with the END token
            current_seq = seq[i]
            end_indices = (current_seq == 0).nonzero()
            if len(end_indices) == 0:
                # If there is no END token, all of seq is part of the length
                length = max_length
            else:
                # The index of the first END token marks the length of the sequence
                length = end_indices[0].cpu().numpy()[0]

            # Skip any sentences starting with the END token
            if (not filter_zerolength) or (length > 0):
                try:
                    valid_sequence_indices.append(i)
                    output_lengths.append(length)
                    output_seq.append(current_seq)
                    output_probs.append(allProbs[i])
                except IndexError:
                    print("INDEX ERROR", i, valid_sequence_indices, output_lengths, length, output_seq, current_seq, output_probs, allProbs)

        # 0=sequence word indices, 1=probabilities, 2=lengths, 3=indices of valid sequence
        return torch.cat([_.unsqueeze(1) for _ in output_seq], 1), torch.cat([_.unsqueeze(1) for _ in output_probs], 1), np.asarray(output_lengths), valid_sequence_indices
...
def sample_gumbel(shape, eps=1e-20):
    ...
    U = torch.FloatTensor(shape).cuda()
    torch.nn.init.uniform(U, a=0, b=1)
