...
def setup(opt):
    ...
    if vars(opt).get('start_from', None) is not None:
        ...
        assert os.path.isfile(os.path.join(opt.start_from,"infos_"+opt.id+postfix+".pkl")),"infos.pkl file does not exist in path %s"%opt.start_from
        model.load_state_dict(torch.load(os.path.join(opt.start_from, 'model'+postfix+'.pth')))
