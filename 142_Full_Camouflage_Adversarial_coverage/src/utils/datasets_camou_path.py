...
from utils.general import check_requirements, xyxy2xywh, xywh2xyxy, xywhn2xyxy, xyn2xy, segment2box, segments2boxes,  xyxy2xywhn,\
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, faces, texture_size, vertices, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            # paths = [i.replace(".txt", ".png") for i in glob.glob(os.path.join(path, "*.txt"))]
            paths = glob.glob(os.path.join(path, "*.txt"))
            self.img_files = sorted(paths)
        ...
        self.label_files = self.img_files  # labels
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        for i, (im_file, lb_file) in enumerate(pbar):
            try:
                ...
                shape = [800,800] #exif_size(im)  # image size
                segments = []  # instance segments
                # assert (shape[0] > 9) & (shape[1s] > 9), f'image size {shape} <10 pixels'
    ...
    def __getitem__(self, index):
        ...
        if self.ret_mask:
            ...
            # mask_file = os.path.join(self.mask_dir, "%s.png" % self.files[index])
        ...
        # 所有有数据的维度向后推了1，类别本来是在第0维，label_out后就在第1维了
...
def load_image(self, index):
    ...
    if self.phase == 'training':
        sa, sb = os.sep + 'train_label_new' + os.sep, os.sep + 'train' + os.sep  # /images/, /labels/ substrings
    else:
