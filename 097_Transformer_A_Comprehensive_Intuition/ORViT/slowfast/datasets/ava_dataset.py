...
class Ava(torch.utils.data.Dataset):
    ...
    def _load_data(self, cfg):
        ...
        def convert_keys(d):
            return {ava_helper.sec_to_frame(k):v for k,v in d.items()}
        self.all_boxes = {k:convert_keys(v) for k,v in boxes_and_labels.items()}
        boxes_and_labels = [
        ...
        if self.cfg.ORVIT.ENABLE:
            self.orvit_boxes = ava_helper.load_orvit_boxes(cfg, self._split)
        self.print_summary()
    ...
    def _images_and_boxes_preprocessing_cv2(self, imgs, boxes):
        ...
        if self._split == "train":  # "train"
            ...
            if self.cfg.AVA.CENTER_CROP_TEST:
                imgs, boxes = cv2_transform.spatial_shift_crop_list(
                    self._crop_size, imgs, 1, boxes=boxes
                )
    ...
    def __getitem__(self, idx):
        ...
        ) # each value is frame idx (with offset=900)
        ...
        boxes = np.array(boxes) # [n_obj_predict, 4]
        # Score is not used.
        boxes = boxes[:, :4].copy()
        ...
        # ORViT Boxes
        if self.cfg.ORVIT.ENABLE:
            orvit_boxes = self.get_orvit_boxes(video_idx, sec, seq) # [T, O, 4]
            T, O, _ = orvit_boxes.shape
            orvit_boxes = orvit_boxes.reshape(-1, 4) # [T*O, 4]
            boxes = np.concatenate([boxes, orvit_boxes], axis = 0) # [n_boxes + T*O, 4]
        ...
        if self.cfg.ORVIT.ENABLE:
            boxes, orvit_boxes = boxes[:-T*O], boxes[-T*O:].reshape(T, O, 4)
            orvit_boxes = torch.from_numpy(orvit_boxes)
            H, W = imgs.shape[-2], imgs.shape[-1]
            orvit_boxes = orvit_boxes / torch.tensor([W, H, W, H]).reshape(1,1,4)
            orvit_boxes = box_ops.box_xyxy_to_cxcywh(orvit_boxes)
            orvit_boxes = box_ops.zero_empty_boxes(orvit_boxes, mode='cxcywh')
            orvit_boxes = orvit_boxes.float()
        ...
        if self.cfg.ORVIT.ENABLE:
            extra_data["orvit_bboxes"] = orvit_boxes
        return imgs, label_arrs, idx, extra_data

    def get_orvit_boxes_provided(self, video_idx, sec, seq, orvit_boxes=None, O = None):
        if orvit_boxes is None:
            orvit_boxes = self.orvit_boxes
        if O is None:
            O = self.cfg.ORVIT.O

        vid_name = self._video_idx_to_name[video_idx]
        seq = [round_frame(sec) for sec in seq]
        
        # Sort
        vid_boxes = [orvit_boxes[vid_name][fid] for fid in seq]
        ...
        out_boxes = np.zeros([len(seq), O, 4])
    ...
    def get_orvit_boxes(self, video_idx, sec ,seq):
        return self.get_orvit_boxes_provided(video_idx, sec, seq)

    def get_orvit_boxes_detected(self, video_idx, sec, seq, orvit_boxes = None, O = None):
        SIZE = 224
        if orvit_boxes is None:
            orvit_boxes = self.orvit_boxes
        if O is None:
        ...
        fids_with_boxes = set([int(k.split('_')[-1][:-4]) for k in orvit_boxes[vid_name][sec].keys()])
        image_paths = [self._image_paths[video_idx][frame] for frame in seq]
        fids = [int(p.split('/')[-1].split('_')[-1][:-4]) for p in image_paths]
        _max = len(seq)
        def get_closest(fid):
            for d in range(_max):
                for direction in [-1, 1]:
                    if fid + d in fids_with_boxes:
                        return fid+d
            return -1
        fids = sorted(list(map(get_closest, fids)))
        fids = [f"{vid_name}/{vid_name}_{fid:06}.jpg" if fid != -1 else None for fid in fids]
        vid_boxes = [orvit_boxes[vid_name][sec][fid] if fid is not None else np.empty(0,5) for fid in fids]
...
def round_frame(sec):
    ...
    if sec < MINF: sec = MINF
    if sec > MAXF: sec = MAXF
    return sec
...
def get_orvit_boxes_file(cfg, mode):
    ...
    if not isinstance(boxes, str):
        ...
        if len(boxes) != 1:
            logger.warning(f"Found more than 1 box list for orvit boxes in {mode} dataloader: {boxes} (use first)")
        boxes = boxes[0]
    return boxes
