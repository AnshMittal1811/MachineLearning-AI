...
def create_dataset(opt, **kwargs):
    return ConfigurableDataLoader(opt, **kwargs)
...
class ConfigurableDataLoader():
    def __init__(self, opt, **kwargs):
        self.opt = opt
        self.kwargs = kwargs
        self.initialize(opt.phase, **kwargs)

    def initialize(self, phase, **kwargs):
        opt = self.opt
        self.phase = phase
        if hasattr(self, "dataloader"):
            del self.dataloader
        dataset_class = find_dataset_using_name(opt.dataset_mode)
        dataset = dataset_class(util.copyconf(opt, phase=phase, isTrain=phase == "train"), **kwargs)
        ...
        self.dataloader = torch.utils.data.DataLoader(
            dataset,
            pin_memory=not opt.dataload_multiproc,
            batch_size=opt.batch_size,
            shuffle=shuffle,
            num_workers=int(opt.num_gpus) if opt.dataload_multiproc else 0,
    ...
    def set_phase(self, target_phase):
        if self.phase != target_phase:
            self.initialize(target_phase, **self.kwargs)
