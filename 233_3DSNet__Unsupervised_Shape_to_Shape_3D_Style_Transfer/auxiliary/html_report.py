...
def main(trainer, outHtml=None):
    ...
    for i in range(3):
        ...
        table.add_row([webpage.mesh(output_mesh["path_a"], normalize=False),
                       webpage.mesh(output_mesh["path_b"], normalize=False)], "inputs")
        table.add_row([webpage.mesh(output_mesh["path_aa"], normalize=False),
                       webpage.mesh(output_mesh["path_bb"], normalize=False)], "reconstruction")
        table.add_row([webpage.mesh(output_mesh["path_ab"], normalize=False),
                       webpage.mesh(output_mesh["path_ba"], normalize=False)], "style_transfer")
    ...
    webpage_after.tables["SMAL"].add_row([
