...
class Logs(object):
    ...
    def update_curves(self, vis, path):
        ...
        for name in self.curves_names:
            if name[:4] == "loss" and np.array(self.curves[name])[-1].item() != 0:
        ...
        vis.line(X=X_Loss,
                 Y=np.log(Y_Loss),
                 win='log',
                 opts=dict(title="log loss", legend=Names_Loss))
        try:
        ...
        try:
            vis.line(X=np.arange(len(self.curves["lpips"])),
                     Y=self.curves["lpips"],
                     win='lpips',
                     opts=dict(title="lpips"))
        except:
            pass
