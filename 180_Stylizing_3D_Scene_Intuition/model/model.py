...
class StyleNeRFpp(nn.Module):
    ...
    def forward(self, ray_o, ray_d, fg_z_max, fg_z_vals, bg_z_vals, style_img, style_idx):
        '''
        :param ray_o, ray_d: [..., 3]
        :param fg_z_max: [...,]
        ...
        :return
        '''
        latent = self.latent_codes(torch.tensor(0).cuda().long())
        
        if self.stage != "first":
            style_mean = self.RAIN_net.get_hyper_input(style_img.cuda().unsqueeze(0))
